# ThumbCommander - a Wing Commander style game for the Thumby family.
#
# Runs unmodified on all three targets:
#   * Thumby      (RP2040, 72x40 1-bit 4-level dither, no audio)
#   * ThumbyColor (RP2350, 128x128 RGB565, IMA-ADPCM audio)
#   * PC          (thumby_engine.pc - a CPython/pygame emulation of the
#                  ThumbyColor firmware; started via tool/run_pc.py)
#
# All platform differences live behind thumby_engine.platform; the
# game-specific tunables live in constants.py.

from thumby_engine.platform import (display, IS_THUMBY_COLOR, Sprite, PC,
                                    create_sprite, play_cutscene_animation,
                                    create_cancel_callback, audio_load,
                                    audio_play, audio_stop, audio_set_loop,
                                    audio_set_volume, audio_get_position,
                                    rumble, buttonA, buttonB, buttonU,
                                    buttonD, buttonL, buttonR, buttonLB,
                                    buttonRB, buttonMENU, dpadPressed,
                                    inputJustPressed)

from thumby_engine.util.fpmath import (int2fp, fp2int, fp2float, float2fp,
                                       fpmul, fpdiv, project, fpsin, fpcos,
                                       rotate_z_x, rotate_z_y, sign,
                                       sort_by_z, apply_physics)

# Game-specific constants (extend the shared PC object)
import constants # noqa: F401
PILOT_PERIOD = PC.FPS // 10
...
if not IS_THUMBY_COLOR:
    ...
    freq(300_000_000)
    play_cutscene_animation("assets/intro_128_80.COL.bin", 21,
                            create_cancel_callback())

# Sprites - using platform-aware loading
OBJECTS = [None, None, None, None, None, None]  # 0=explode, 1=astroid1, 2=astroid2, 3=enemyA, 4=enemyB, 5=wingman

# Load sprites with platform-appropriate versions
OBJECTS[0] = create_sprite(32, 31, ("assets/explode_32_31.BIT.bin", "assets/explode_32_31.SHD.bin"), 0, 0, 0, cWidth=56, cHeight=54)
OBJECTS[1] = create_sprite(32, 27, ("assets/astroid1_32_27.BIT.bin", "assets/astroid1_32_27.SHD.bin"), 0, 0, 0, cWidth=56, cHeight=47)
OBJECTS[2] = create_sprite(32, 27, ("assets/astroid2_32_27.BIT.bin", "assets/astroid2_32_27.SHD.bin"), 0, 0, 0, cWidth=56, cHeight=47)
SHIP_NAMES = ["enemy1", "enemy2", "wingman"]
...
def load_enemy_sprites(type_indices):
    ...
    for slot in (3, 4):
        ...
        OBJECTS[slot] = create_sprite(40, 34, ("assets/" + name + "_40_34.BIT.bin", "assets/" + name + "_40_34.SHD.bin"), 0, 0, 0, cWidth=70, cHeight=59)
...
class Enemies:
    def __init__(self, num=1, front=None, behind=None):
        ...
        self.shieldSprite = create_sprite(40, 40, ("assets/shield_40_40.BIT.bin", "assets/shield_40_40.SHD.bin"), 0, 0, 0, cWidth=70, cHeight=70)
...
class Ship:
    def __init__(self):
        ...
        if IS_THUMBY_COLOR:
            ...
            self.stick_back_sprite = create_sprite(28, 16, "assets/stick_back_28_16.COL.bin", PC.SHIP_X+45, PC.SHIP_Y+38, 0)
            self.stick_forward_sprite = create_sprite(28, 16, "assets/stick_forward_28_16.COL.bin", PC.SHIP_X+45, PC.SHIP_Y+36, 0)
            self.target_sprite = create_sprite(24, 24, "assets/target_24_24.COL.bin", PC.CENTER_X-12, PC.CENTER_Y-12, 0)
            self.target_active_sprite = create_sprite(24, 24, "assets/targetactive_24_24.COL.bin", PC.CENTER_X-12, PC.CENTER_Y-12, 0)
            self.radar_sprite = create_sprite(24, 24, "assets/radar_24_24.COL.bin", PC.SHIP_X + PC.RADAR_X, PC.SHIP_Y + PC.RADAR_Y, 0)
            self.radar_frame = 0
            self.radar_framecount = self.radar_sprite.frameCount - 1
            self.fx = FXEngine()
        else:
            # Load Grayscale sprites
            self.cockpit_sprite = create_sprite(66, 18, ("assets/cockpit_66_18.BIT.bin", "assets/cockpit_66_18.SHD.bin"), PC.SHIP_X, PC.SHIP_Y, 1)
            self.target_sprite = create_sprite(7, 7, ("assets/target_7_7.BIT.bin", "assets/target_7_7.SHD.bin"), PC.CENTER_X-3, PC.CENTER_Y-3, 0)
            self.target_active_sprite = create_sprite(7, 7, ("assets/targetactive_7_7.BIT.bin", "assets/targetactive_7_7.SHD.bin"), PC.CENTER_X-3, PC.CENTER_Y-3, 0)
            self.radar_sprite = create_sprite(15, 15, ("assets/radar_15_15.BIT.bin", "assets/radar_15_15.SHD.bin"), PC.RADAR_X, PC.RADAR_Y, 0)
            self.fx = None
    ...
    def run(self):
        ...
        if IS_THUMBY_COLOR:
            ...
            dx = self.cockpit_sprite_x - PC.SHIP_X
            dy = self.cockpit_sprite_y - PC.SHIP_Y
    ...
    @micropython.native
    def _move_me_actions(self, enemies, new_time):
        global player_speed, player_target_speed

        b = self._button_states
        sr = SHIFT_REQUIRED
        shift = b[KEY_SHIFT].pressed()

        if b[KEY_TARGET_NEXT].justPressed() and sr[KEY_TARGET_NEXT] == shift:
            self._cycle_target(enemies, 1)
            return 2
        elif b[KEY_TARGET_PREV].justPressed() and sr[KEY_TARGET_PREV] == shift:
            self._cycle_target(enemies, -1)
            return 2
            ...
            return 2
        elif b[KEY_BREAK].justPressed() and sr[KEY_BREAK] == shift:
            player_speed = 1<<16
            return 2
        elif b[KEY_EJECT].pressed() and sr[KEY_EJECT] == shift:
            return 1
        return 0

    @micropython.native
    def _move_me_motion(self, new_time, t, first):
        ...
        if first:
            b = self._button_states
            if b[KEY_MOVE_RIGHT].pressed():
                player_angle[0] -= 1<<16; player_angle[2] = -3; cx = PC.SHIP_X-1
            elif b[KEY_MOVE_LEFT].pressed():
                player_angle[0] += 1<<16; player_angle[2] = 3; cx = PC.SHIP_X+1
            elif b[KEY_MOVE_DOWN].pressed():
                player_angle[1] -= 1<<16; cy = PC.SHIP_Y-1
            elif b[KEY_MOVE_UP].pressed():
                player_angle[1] += 1<<16; cy = PC.SHIP_Y+1
    ...
    def move_me(self, enemies):
        ...
        actions = self._move_me_actions(enemies, new_time)
        if actions == 1:
            return False
        self._move_me_motion(new_time, t, actions == 0)
        return True
...
def launch():
    ...
    display.update()
    if IS_THUMBY_COLOR:
        play_cutscene_animation("assets/start_128_56.COL.bin", 10, create_cancel_callback())
    else:
        play_cutscene_animation("assets/start1_74_30.BIT.bin", 20, create_cancel_callback())
        display.fill(PC.BLACK)
        display.update()
        play_cutscene_animation("assets/start2_74_30.BIT.bin", 20, create_cancel_callback())
    sleep(0.25)
...
def die():
    display.fill(PC.BLACK)
    if IS_THUMBY_COLOR:
        play_cutscene_animation("assets/medie_128_110.COL.bin", 15, create_cancel_callback())
    else:
        play_cutscene_animation("assets/medie_74_40.BIT.bin", 6, create_cancel_callback())
    sleep(0.3)
...
def home():
    display.fill(PC.BLACK)
    if IS_THUMBY_COLOR:
        play_cutscene_animation("assets/home_128_80.COL.bin", 20, create_cancel_callback())
    else:
        play_cutscene_animation("assets/home_74_30.BIT.bin", 10, create_cancel_callback())
    sleep(0.3)
    collect()  # Clean up after animation
...
def eject():
    display.fill(PC.BLACK)
    if IS_THUMBY_COLOR:
        play_cutscene_animation("assets/eject_128_80.COL.bin", 20, create_cancel_callback())
    else:
        play_cutscene_animation("assets/eject_72_40.BIT.bin", 10, create_cancel_callback())
    sleep(0.3)
    collect()  # Clean up after animation

def game_over():
    display.fill(PC.BLACK)
    display.setFont("assets/font8x8.bin", 8, 8, 1)
...
def menu():
    ...
    display.setFont(PC.FONT_FILE, PC.FONT_WIDTH, PC.FONT_HEIGHT, PC.FONT_SPACE)
    if IS_THUMBY_COLOR:
        audio_load("assets/menu_background.ima")
        audio_play()
        audio_set_loop(True, 150496, 759838)
        menu_sprite = "assets/menu_128_120.COL.bin"
    else:
        menu_sprite = create_sprite(71, 40, ("assets/menu_71_40.BIT.bin", "assets/menu_71_40.SHD.bin"), 2, 0, 0)
...
if IS_THUMBY_COLOR:
    ...
    import color_enhancements
    color_enhancements.apply(globals())
    collect()
    print(f"After cleanup: {mem_free()}")
    play_cutscene_animation("assets/title_128_80.COL.bin", 21, create_cancel_callback())
else:
    # Intro finished
    intro.finish()
...
def main():
    global lifes, player_speed, player_target_speed, player_angle, score, hudShip
    while True:
        lifes = 5
        player_speed = player_target_speed = float2fp(1)
        player_angle = [0, 0, 0]
