...
class PWM:
    ...
    def _audio_player_thread():
        ...
        while not PWM._stop_playback:
            ...
            if chunk and pygame:
                chunks_played += 1
                if chunks_played <= 5 or chunks_played % 100 == 0:
                    with PWM._lock:
                        buf_len = len(PWM._sample_buffer)
                    print(f"[Audio] Playing chunk #{chunks_played}: {len(chunk)} samples, buffer: {buf_len}")
                try:
                    # Verify mixer is still initialized before playing
                    mixer_info = pygame.mixer.get_init()
                    if mixer_info is None:
                        print(f"[Audio] Error playing chunk #{chunks_played}: mixer not initialized", flush=True)
                        # Try to reinitialize?
                        break

                    # Verify channel is still valid
                    if PWM._channel is None:
                        print(f"[Audio] Error playing chunk #{chunks_played}: channel is None", flush=True)
                        break
                    ...
                    if not PWM._channel.get_busy():
                        ...
                        if chunks_played <= 5:
                            print(f"[Audio] Played chunk #{chunks_played}, buffer={buf_len}", flush=True)
                    else:
                        PWM._channel.queue(sound)
                        if chunks_played <= 5:
                            print(f"[Audio] Queued chunk #{chunks_played}, buffer={buf_len}", flush=True)

                except Exception as e:
                    print(f"[Audio] Error playing chunk #{chunks_played}: {e}", flush=True)
                    import traceback
                    traceback.print_exc()
                    # Continue trying to play other chunks
            else:
...
class Timer:
    ...
    def _periodic_worker(self):
        ...
        Uses perf_counter for accurate scheduling instead of Event.wait()
        """
        ...
        while not self._stop_event.is_set():
            ...
            # High-precision sleep until next scheduled time
    ...
    def _oneshot_worker(self):
        """
        Worker thread for one-shot callbacks with high-precision timing
